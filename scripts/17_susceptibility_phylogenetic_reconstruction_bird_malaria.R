...
list_results <- read_rds("data-raw/bird_plasmodium_1000_sims.rds")

plasmodium_prediction <- list_results %>%
...
birds_tree_tip_names <- birds_tree$tip.label %>%
  ...
  left_join(plasmodium_prediction) %>% na.exclude()
...
{
  png(file = "data-raw/malaria_susceptible_tree.png",
